...
function updateGamepadStatus() {
  ...
  for (var i in gamepads) {
    ...
    if (gp) {
      ...
      var leftX = gp.axes[0];
      var leftY = gp.axes[1];
      var rightX = gp.axes[2];
      var rightY = gp.axes[3];

      console.log(`Left Stick: (${leftX}, ${leftY}), Right Stick: (${rightX}, ${rightY})`);

      // 更新左摇杆轮盘位置
      var leftStick = document.getElementById("left-stick").querySelector(".stick-indicator");
      leftStick.style.transform = `translate(${leftX * 50}%, ${leftY * 50}%) translate(-50%, -50%)`;

      // 更新右摇杆轮盘位置
      var rightStick = document.getElementById("right-stick").querySelector(".stick-indicator");
      rightStick.style.transform = `translate(${rightX * 50}%, ${rightY * 50}%) translate(-50%, -50%)`;
    }
  }
